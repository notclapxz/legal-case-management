...
-- Paso 8: Crear trigger simple para timestamps
CREATE OR REPLACE FUNCTION actualizar_timestamp() RETURNS TRIGGER AS LANGUAGE plpgsql IMMUTABLE AS
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
END;

DROP TRIGGER IF EXISTS trigger_actualizar_timestamp ON casos;
